...
#define STATUSBAR_TEXT_BUFFER_SIZE (2040)

static char stext[STATUSBAR_TEXT_BUFFER_SIZE] = {'\0'};
static int scanner;
static int screen;
...
void buttonpress(XEvent *e) {
  ...
  if (ev->window == selmon->barwin) {
    ...
    if (i < LENGTH(tags) && (drawtagmask & DRAWCLASSICTAGS)) {
      ...
      click = ClkWinTitle;
      // bartabcalculate(selmon, x, TEXTW(stext) - lrpad + 2, ev->x,
      // battabclick);
    }
...
void cleanup(void) {
  ...
  for (i = 0; i < LENGTH(colors) + 1; i++) // +1 for status2d
    free(scheme[i]);
...
void drawbar(Monitor *m) {
  ...
  if ((w = m->ww - tw - x) > bh) {
    if (m->sel) {
      ...
      // Make sure we don't overlap on tags
  ...
  for (j = 0, i = 0; j < tagrows; j++) {
    ...
    for (k = 0; k < columns && i < LENGTH(tags); k++, i++) {
      ...
      XSetForeground(drw->dpy, drw->gc,
                     !invert ? scheme[SchemeSel][ColBg].pixel
      ...
      if (occ & 1 << i) {
        XSetForeground(drw->dpy, drw->gc,
                       !invert ? scheme[SchemeSel][ColFg].pixel
                               : scheme[SchemeNorm][ColBg].pixel);
        XFillRectangle(dpy, drw->drawable, drw->gc, x + 1 + h / 4,
                       y + 1 + h / 4, h / 2, h / 2);

        // XDrawArc(dpy, drw->drawable, drw->gc, x + 1 + h/4, y + 1 + h/4, h/2,
        // h/2, 0, 360*64);
      }
      x += h;
      if (x > max_x) {
...
void get_color_from_pywal(char *out) {
  ...
  fread(buffer, sizeof(char), len, fi);
  fclose(fi);
...
void setup(void) {
  ...
  colors[SchemeSel][1] = buff;
  colors[SchemeSel][2] = buff;
  colors[SchemeTagsSel][1] = buff;
  colors[SchemeInfoSel][1] = buff;
